...
namespace SetupParameterLibrary
{
    public static class SetupParameterHandler
    {
        ...
        public static IList<SetupSubGroup> GetSubGroupsByGroup(Guid groupId)
        {
            using (var context = new SetupDbContext())
            {
                return context.SubGroups.Where(s=>s.SetupGroupId == groupId).AsEnumerable().Select(s => new SetupSubGroup(s)).ToList();
            }
        }
        ...
        public static SetupSubGroup SaveSubGroup(SetupSubGroup subGroup)
        {
            using (var context = new SetupDbContext())
            {
                ...
                context.SaveChanges();
                var newModel = context.SubGroups.FirstOrDefault(s => s.Name == subGroup.Name);
        ...
        public static SetupParameter SaveParameter(SetupParameter parameter)
        {
            using (var context = new SetupDbContext())
            {
                ...
                context.SaveChanges();
                var newModel = context.Parameters.FirstOrDefault(p => p.Name == parameter.Name);
